...
#  include "box.h"
   ...
   call rdprm1(title(1:80),natom,ntypes,nbonh,mbona,ntheth,&
                mtheta,nphih,mphia,nhparm,nparm,nnb,nres,nbona,&
                ntheta,nphia,numbnd,numang,nptra,natyp,nphb,   &
                ifbox,nmxrs,ifcap,numextra)
   !ifpert is not implemented in the data structure
   ! as well as nbper,ngper,ndper,mbper,mgper,mdper
   !ncopy is also not implemented
   if (nparm == 1) then
...
subroutine rdparm2(x,ix,ih,ipairs,i_stack)
   implicit none
   _REAL_ x(*)
...
!mjhsieh HBCUT is not in the sff struct
!  
...
!  read(nf,fmt) (ix(i+i66-1),i=1,natom)
   call rdprm2(ih(m04),x(l15),x(lwinv),ix(i04),&
            ix(i08),ntype,ix(i06),ih(m02),ix(i02),&
            rk(1),req(1),tk(1),teq(1),pk(1),pn(1),phase(1),solty(1),&
            nttyp,cn1(1),cn2(1),&
            ...
            ix(i10),asol(1),bsol(1),&
            ih(m06),ih(m08),ix(i64),      &
            ix(i66))
   !print *,ix(i66:i66+natom-1)
   ix(i02+nres) = natom+1
...
!     read(nf,fmt) (ix(i+i70-1),i=1,nspm)
      call rdprm3(iptres,nspm,nspsol,ix(i70))
      !call mexit(6,0)
!     
!     fmtin = rfmt
!     type = 'BOX_DIMENSIONS'
...
!     read(nf,fmt) cutcap,xcap,ycap,zcap
      call rdprm4(natcap,cutcap,xcap,ycap,zcap)
   else
      natcap = 0
      cutcap = 0.0d0
      xcap = 0.0d0
...
!     read(nf,fmt) (x(l96+i-1),i=1,natom)
   ...
   if( icfe /= 0 ) then
      ...
      if( ifpert == 0 ) then
         ...
         write(6,'(a)') 'Pert is not implemented with SFF'
         call mexit(6,1)
      end if
...
#ifdef CHARMM
   write(6,'(a)') 'CHARMM is not supported by SFF framework'
   call mexit(6,1)
!  
