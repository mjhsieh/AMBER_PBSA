...
#include "box.h"
   ...
   deallocate(   rmin, stat = alloc_err(11) )
   deallocate(   radi, stat = alloc_err(12) )
   deallocate(  radip, stat = alloc_err(13) )
   deallocate( radip2, stat = alloc_err(14) )
   deallocate( radip3, stat = alloc_err(15) )
   deallocate( nzratm, stat = alloc_err(16) )
   deallocate(   nmax, stat = alloc_err(17) )
   deallocate(   nexp, stat = alloc_err(18) )
   deallocate(sumnmax, stat = alloc_err(19) )
   deallocate(sumnexp, stat = alloc_err(20) )
   deallocate( avnmax, stat = alloc_err(21) )
   deallocate( avnexp, stat = alloc_err(22) )
   deallocate(   scrd, stat = alloc_err(23) )
   ...
   deallocate(  iar1pb, stat = alloc_err(24) )
   deallocate( iprshrt, stat = alloc_err(25) )
   deallocate( iprlong, stat = alloc_err(26) )
...
#  include "pb_md.h"
   ...
   if ( pbverbose ) then
      write(6, *)
      write(6, *) '======== Implicit Solvent Initialization ========'
      write(6, *)
      write(6,'(5x,a,3i9)') 'Max Nonbonded Pairs:', maxnbr, maxnba, maxmax
      write(6, *)
   endif
   ...
   if ( ifcap == 0 ) then
      ...
      ! initialization for for vdw surface
         ...
         enddo
         if ( pbverbose ) write(6, *) ' VDW Surface: found nonzero radii', nsatm
      end if
