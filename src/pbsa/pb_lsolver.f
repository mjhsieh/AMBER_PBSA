...
subroutine init_array( solvopt,epsx,epsy,epsz,p_bv,p_iv,p_xs )
   ...
   select case (solvopt)
   case (1)
      l_am1(1-l_xmym:0) = l_epsout
      l_am2(1-l_xmym:0) = l_epsout
      l_am3(1-l_xmym:0) = l_epsout
      l_am1(1:l_xmymzm) = epsx(1:l_xmymzm)
      l_am2(1:l_xmymzm) = epsy(1:l_xmymzm)
      l_am3(1:l_xmymzm) = epsz(1:l_xmymzm)
      ...
      if (l_pbkappa == ZERO) then
         do l = 1, l_xmymzm
            l_ad(l) = l_am1(l-1   ) + l_am1(l     ) + l_am2(l-l_xm  ) + l_am2(l     ) + l_am3(l-l_xmym) + l_am3(l     )
         end do
      else
   ...
   case (2)
      ...
      lepsx(1:l_xmymzm) = epsx(1:l_xmymzm)
      lepsy(1:l_xmymzm) = epsy(1:l_xmymzm)
      lepsz(1:l_xmymzm) = epsz(1:l_xmymzm)
