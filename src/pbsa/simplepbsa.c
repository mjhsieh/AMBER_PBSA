...
int main(int argc, const char **argv){
	...
	gopt_option( 913, GOPT_ARG, gopt_shorts( 0 ), gopt_longs( "cutres" )),
    ...
    if( gopt_arg( options, 913, &tmpstring ) > 0 )
       opt->cutres=atof(tmpstring);
    ...
    e_pb = epbsa(ipb,opt,prm,x,
		 grad,&evdw,&eelt,&esurf,
		 &edisp);
