...
#  include "md.h"
   ...
   if ( imin == 1) then ! reused to print minimization results
      etot   = ener(1)
      enonb  = ener(2)
      eel    = ener(3)
      ehbond = ener(4)
...
#  include "pb_md.h"
   ...
   write(6, '(4x, f10.3, f10.3)') gox, gox + (xm+1) * h
   write(6, '(4x, f10.3, f10.3)') goy, goy + (ym+1) * h
   write(6, '(4x, f10.3, f10.3)') goz, goz + (zm+1) * h
   write(6, *)
   ...
   write(6, *)
   if ( ipb == 1 ) then
      write(6, *) '  Use probe-based SES definition'
